...
<?php
$sql    = 'SELECT  *,query.workcode as "work-code",query.category as category,query.subcategory as query,servicetype.name as name,servicetype.name as name FROM db_wms_archive,query,servicetype WHERE contactnumber="'.$_REQUEST["search1"].'" AND query.workCode=db_wms_archive.workCode AND servicetype.stype=db_wms_archive.stype ORDER BY date DESC';
